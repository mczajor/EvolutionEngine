...
public class MapVisualizer {
     ...
     */
    public String draw(MoveVector lowerLeft, MoveVector upperRight) {
        ...
        for (int i = upperRight.y + 1; i >= lowerLeft.y - 1; i--) {
            ...
            for (int j = lowerLeft.x; j <= upperRight.x + 1; j++) {
                if (i < lowerLeft.y || i > upperRight.y) {
                    ...
                    if (j <= upperRight.x) {
                        builder.append(drawObject(new MoveVector(j, i)));
                    }
                }
            }
    ...
    private String drawHeader(MoveVector lowerLeft, MoveVector upperRight) {
        StringBuilder builder = new StringBuilder();
        builder.append(" y\\x ");
        for (int j = lowerLeft.x; j < upperRight.x + 1; j++) {
    ...
    private String drawObject(MoveVector currentPosition) {
        String result = null;
